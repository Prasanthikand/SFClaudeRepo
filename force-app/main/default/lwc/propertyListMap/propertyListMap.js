...
export default class PropertyListMap extends LightningElement {
    ...
    displayProperties() {
        if (this.leafletState !== LEAFLET_READY) return;
        if (this.propertyLayer) { this.map.removeLayer(this.propertyLayer); }
        ...
        const markers = this.properties.map((property) => {
            const latLng = [property.Location__Latitude__s, property.Location__Longitude__s];
