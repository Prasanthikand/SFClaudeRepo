...
describe('c-property-tile', () => {
    ...
    it('displays a property in the tile', () => {
        ...
        expect(headerEl.textContent).toBe(`${PROPERTY.City__c} • ${PROPERTY.Name}`);
        const paragraphEl = element.shadowRoot.querySelector('p');
        expect(paragraphEl.textContent).toBe(`Beds: ${PROPERTY.Beds__c} - Baths: ${PROPERTY.Baths__c}`);
        const priceEl = element.shadowRoot.querySelector('lightning-formatted-number');
    ...
    it('displays the correct background image in the tile', () => {
        ...
        expect(backgroundEl.style.backgroundImage).toBe(`url(${PROPERTY.Thumbnail__c})`);
    });
